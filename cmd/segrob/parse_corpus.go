...
func parseCorpusPublishArgs(args []string, ui UI) (CorpusPublishOptions, error) {
	...
	fs.Usage = func() {
		...
		_, _ = fmt.Fprintf(w, "\nArguments:\n")
		_, _ = fmt.Fprintf(w, helpArgFmt, "id", "Document ID to publish (omit to publish all ACKed)")
		_, _ = fmt.Fprintf(w, "\nOptions:\n")
...
func parseCorpusPushTxtArgs(args []string, ui UI) (CorpusPushTxtOptions, error) {
	...
	fs.Usage = func() {
		...
		_, _ = fmt.Fprintf(w, "\nArguments:\n")
		_, _ = fmt.Fprintf(w, helpArgFmt, "id", "Document ID")
		_, _ = fmt.Fprintf(w, helpArgFmt, "file", "Path to the plain text file")
		_, _ = fmt.Fprintf(w, "\nOptions:\n")
...
func parseCorpusSetLabelArgs(args []string, ui UI) (CorpusSetLabelOptions, error) {
	...
	fs.Usage = func() {
		...
		_, _ = fmt.Fprintf(w, "  Recommended format: prefix:value (e.g., creator:garcía_lorca).\n")
		_, _ = fmt.Fprintf(w, "  The following prefixes are displayed as columns by 'corpus ls':\n")
		_, _ = fmt.Fprintf(w, "    creator:  title:  translator:  date:  language:\n\n")
		_, _ = fmt.Fprintf(w, "Arguments:\n")
		_, _ = fmt.Fprintf(w, helpArgFmt, "doc_id", "ID of the document")
		_, _ = fmt.Fprintf(w, helpArgFmt, "label", "One or more labels to add/remove")
		_, _ = fmt.Fprintf(w, "\nOptions:\n")
...
func parseCorpusLsLabelArgs(args []string, ui UI) (CorpusLsLabelOptions, error) {
	...
	fs.Usage = func() {
		...
		_, _ = fmt.Fprintf(w, "\nArguments:\n")
		_, _ = fmt.Fprintf(w, helpArgFmt, "id", "Document ID to list labels for (omit to list all)")
		_, _ = fmt.Fprintf(w, "\nOptions:\n")
		printOpt(w, "--db", "PATH", "Path to corpus SQLite file (or SEGROB_CORPUS_DB)")
